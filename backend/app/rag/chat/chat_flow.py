            ...
            if db_host_name and isinstance(e, Exception) and "Only ws:// or wss://" in str(e):
                try:
                    ...
                    if summary_mode:
                        ...
                        tables_md = rows_to_markdown(tables_rows, ["table", "exec_count", "total_s"])
                        statement_sql = build_statement_summary_query(start_ts, end_ts)
                        stmt_rows = normalize_rows(run_managed_mcp_db_query(db_host_name, statement_sql), logger=logger)
                        stmt_table_rows = []
                        for r in stmt_rows[:10]:
                            if isinstance(r, dict):
                                schema_name = str(r.get("agg_schema_name") or "")
                        ...
                        recommendations_text = build_rule_recommendations(digest_rows, stmt_rows)
                        text = (
            ...
            if fallback_name:
                try:
                    ...
                    if summary_mode:
                        ...
                        tables_md = rows_to_markdown(tables_rows, ["table", "exec_count", "total_s"])
                        statement_sql = build_statement_summary_query(start_ts, end_ts)
                        stmt_rows = normalize_rows(run_managed_mcp_db_query(fallback_name, statement_sql), logger=logger)
                        stmt_table_rows = []
                        for r in stmt_rows[:10]:
                            if isinstance(r, dict):
                                schema_name = str(r.get("agg_schema_name") or "")
